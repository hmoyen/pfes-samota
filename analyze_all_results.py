...
def print_per_req_table(all_metrics: dict):
    ...
    print(f"\n{'='*80}")
    print("PER-REQUIREMENT VIOLATIONS (mean violations/run)")
    print(f"{'='*80}")
...
def print_stat_tests_detail(stat_tests: dict):
    ...
    print(f"\n{'='*80}")
    print("STATISTICAL TESTS: SAMOTA vs Baselines (Mann-Whitney U, Vargha-Delaney A_AB)")
    print("  * = p < 0.05 (statistically significant)")
    print("  A_AB > 0.5 means SAMOTA finds more violations")
    print(f"{'='*80}")
