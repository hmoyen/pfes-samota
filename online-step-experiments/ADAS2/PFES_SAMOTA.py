...
LOCAL SEARCH:
  - Trains an RBF surrogate per cluster, then runs GA for 200 generations
    to find the local optimum (an earlier version just added noise to the
    best point, which didn't work well)
...
DATA TRACKING:
  ...
  - NOTE: raw point_estimates were tried for both surrogate input and violation
    detection but performed 60% worse, so constraint-mapped values are used instead
...
def evaluate_test_case(params):
    ...
    Rationale: raw outputs were tried and showed 60% worse performance
    (15 violations vs 37 baseline). Constraint-mapped values are symmetric around
    the violation boundary (0), making them easier for surrogates to model.
    """
...
def local_search_phase(X_all, F_all, uncovered_objectives, eta_percent=20, l_max=200, n_clusters=20, seed=0):
    ...
    for obj_idx in uncovered_objectives:
        ...
        for cluster_idx in cluster_indices:
            ...
            # Create a single RBF per cluster per objective (not an ensemble)
...
def pfes_samota(max_iterations=1000, max_time_seconds=float("inf"), budget=900, seed=0, logdir=None, window_size=None):
    ...
    for iteration in range(max_iterations):
        ...
        if elapsed_time > max_time_seconds:
            logger.warning(f"TIME LIMIT REACHED at iteration {iteration}")
        ...
        logger.info("Objective status (min score per objective):")
        ...
        print("  Global Search (GS) - PER-OBJECTIVE (one surrogate+GA per uncovered obj)...")
